...
In practice, much of the Create, Read, Update, Delete behavior does not belong in a public Application Programming Interface at all. It belongs in views or other workflow-oriented handlers that can manage the full chain of dependent actions safely. For example, building a connection is slow and may involve multiple systems. The better approach would be for an Application Programming Interface to request a connection, record that request in a `requested` state, and then wait while the underlying work happens. After some time, once the connection has actually been built or rejected, the result should be posted back to a callback endpoint. That approach matches the real-world shape of the operation much better than pretending that a connection can be created immediately by a single synchronous Create action.

### Current repository structure
The repository is organized to support that proof-of-concept work and its documentation:
- `src/` contains the Django code, including the current web views, templates, static assets, models, and the unit tests. The future Application Programming Interface will also live in this code area when it is added.
